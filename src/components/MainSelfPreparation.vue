<template>
    <div id="main-self-preparation">
      ...
      <div v-show="indexTitle === 0" class="ago-container">
        <div
          ...
          :class="[showAnimation ? 'floatAnimation' : 'downAndGrow', arrayDoneSubj[index] ? 'disabled' : '']"
          @click="circleBtn"
          >
      ...
      <button v-if="counter === 3" class="nextBtn" @click="nextBtn">המשך</button>
    </div>
  </template>
  ...
  export default {
    ...
    data() {
      return {
        arrayTitle: [
          ...
          "המשוב האישי מתבסס על שני עקרונות על:",
          "עקרונות היסוד של המשוב - תמיכה באגו.",
          "עקרונות היסוד של המשוב - תמיכה באגו.",
          "עקרונות היסוד של המשוב - איסוף מקסימום מידע מהנחנך.",
        ],
        arrayInfo: [
          "- לחצ/י על תחום -",
          "",
          "המשוב בעצם מהותו, נתפס כמאיים ביותר. הסיבות לכך הן:",
          "על מנת לבטל את הגורמים המאיימים על אגו הנחנך, נקפיד על העקרונות הבאים:",
          "בתור חונכים עלינו לאסוף מקסימום מידע מביצועי הנחנך. הנחנך תמיד מחזיק במידע שאצלך כחונך לא יהיה: שיקוליו, כיצד נראה הביצוע מנקודה מבטו, תחושותיו וחוויתו. ",
          "אז מה עושים?",
        ],
        arrayCircle: [
          "תכנון מתווה המשימה",
    ...
    methods: {
      circleBtn(event) {
        console.log('hi');
    this.arrayDoneSubj[ event.currentTarget.id] = true;
  },
      setStartAnimation() {
